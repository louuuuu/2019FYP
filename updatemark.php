...
if (isset($_POST["submit"])) {
    ...
    // 2
    $query  = "UPDATE mark SET id = '$id', 
    mark = '$mark'";
    $query .= "WHERE id = $id";
 
    mysqli_query($connection, $query);
    
    echo "<script>alert('Update successful!!');window.location.href='score_ad.php';</script>";
    //echo $query;
}
?>
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow">
              ...
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Modify Score</h4>
                      <?php
                          $id=$_GET['id'];
                      ?>

                    <form class="forms-sample" action="updatemark.php" method="post">
                    <input type="hidden" name="id" value="<?php echo $id; ?>">
                      <div class="form-group">
                        <label for="exampleInputName1">Score</label>
                        <input type="INT" class="form-control" name="mark" placeholder="Score">
                      </div>             
                        


                      <button type="submit" name="submit" class="btn btn-primary mr-2">Submit</button>
                      <button class="btn btn-light"><a href="score_ad.php">Cancel</a></button>
                    </form>
                  </div>
                </div>
              </div>
